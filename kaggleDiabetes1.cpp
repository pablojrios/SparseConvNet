	...
	testSet.summary();
	{
		...
		for (epoch++; epoch <= 5; epoch++) {
			...
			cnn.processDatasetRepeatTest(validationSet, batchSize, 1,
					"kaggleDiabetes1.predictions", "",
					"confusionMatrix1.train"); //Monitor progress during training
		}
		...
		cnn.processDatasetRepeatTest(trainSetAsTestSet, batchSize, 12,
				...
				"", "confusionMatrix1.train");
		cnn.processDatasetRepeatTest(testSet, batchSize, 12,
				"Data/kaggleDiabeticRetinopathy/kaggleDiabeticRetinopathyCompetitionModelFiles/kaggleDiabetes1_epoch65.test");
	}
}
