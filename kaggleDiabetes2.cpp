...
Picture* OpenCVPicture::distort(RNG& rng, batchType type) {
	...
	float c00 = 1, c01 = 0, //2x2 identity matrix---starting point for calculating affine distortion matrix
			c10 = 0, c11 = 1;
	float r, alpha, beta;
	if (type == TRAINBATCH) {
...
public:
	Imagenet(int dimension, ActivationFunction fn, int nInputFeatures,
			int nClasses, int cudaDevice = -1, int nTop = 1);
};
Imagenet::Imagenet(int dimension, ActivationFunction fn, int nInputFeatures,
		int nClasses, int cudaDevice, int nTop) :
...
int main() {
	std::string baseName =
			"Data/kaggleDiabeticRetinopathy/kaggleDiabeticRetinopathyCompetitionModelFiles/kaggleDiabeticRetinopathy2";
	...
	testSet.summary();
	{
		Imagenet cnn(2, VLEAKYRELU, trainSet.nFeatures, trainSet.nClasses,
				cudaDevice);

		if (epoch == 0) {
			SpatiallySparseDataset trainSubset = trainSet.subset(
					20 * batchSize);
		...
		//TODO:for (epoch++;epoch<=84;epoch++) {
		for (epoch++; epoch <= 3; epoch++) {
			std::cout << "epoch: " << epoch << std::endl;
			for (int i = 0; i < 3; ++i) {
				//TODO:SpatiallySparseDataset trainSubset=trainSet.subset(12000);
				SpatiallySparseDataset trainSubset = trainSet.subset(70);
				cnn.processDataset(trainSubset, batchSize,
						0.003 * exp(-epoch * 0.03), 0.999);
				cnn.saveWeights(baseName, epoch);
			}
			cnn.processDatasetRepeatTest(validationSet, batchSize, 1,
					"kaggleDiabetes2.predictions", "",
					"confusionMatrix2.train"); //Monitor progress during training
		}
		cnn.processDatasetRepeatTest(validationSet, batchSize, 12,
				"Data/kaggleDiabeticRetinopathy/kaggleDiabeticRetinopathyCompetitionModelFiles/kaggleDiabetes2_epoch84.validation",
				"", "confusionMatrix2.validation");
		...
		cnn.processDatasetRepeatTest(trainSetAsTestSet, batchSize, 12,
				"Data/kaggleDiabeticRetinopathy/kaggleDiabeticRetinopathyCompetitionModelFiles/kaggleDiabetes2_epoch84.train",
				"", "confusionMatrix2.train");
		cnn.processDatasetRepeatTest(testSet, batchSize, 12,
				"Data/kaggleDiabeticRetinopathy/kaggleDiabeticRetinopathyCompetitionModelFiles/kaggleDiabetes2_epoch84.test");
	}
}
